...
public sealed class XsltGeneratorService : IXsltGeneratorService
{
    ...
    private string BuildBody(BlockTreeDto tree)
    {
        ...
        foreach (var section in tree.Sections.OrderBy(s => s.Order))
        {
            ...
            // Sütun gruplu render: herhangi bir blokta col atanmışsa kullan
            if (blockList.Any(b => b.Layout?.Col.HasValue == true))
            {
                RenderColGrouped(sb, blockList, tree);
                continue;
            }
    ...
    // Sütun gruplu render: blockList içindeki bloklar col değerine göre gruplandırılır,
    // her grup aynı <tr> içinde ayrı <td> olarak render edilir.
    // Aynı sütundaki (aynı col) birden fazla blok üst üste yığılır.
    private void RenderColGrouped(StringBuilder sb, List<BlockDto> blockList, BlockTreeDto tree)
    {
        var colGroups = blockList
            .GroupBy(b => b.Layout?.Col ?? 0)
            .OrderBy(g => g.Key)
            .ToList();
        ...
        foreach (var group in colGroups)
        {
            ...
            var colWidth = firstBlock.Layout?.Width ?? "1/3";
            var colCss = FractionCss(colWidth);
            sb.AppendLine($"      <td style=\"width:{colCss};vertical-align:top;\">");
            foreach (var b in group)
            {
                var blockAlign = TextAlignStyle(b.Layout?.Alignment);
                // Her bloğu display:block div ile sar — aynı sütundaki blokların üst üste
                // (yatay değil dikey) dizilmesini garanti et.
                sb.AppendLine($"        <div style=\"display:block;width:100%;{blockAlign}\">{DispatchBlock(b, tree)}</div>");
            }
            sb.AppendLine("      </td>");
            totalWidth += FractionValue(colWidth);
        }

        if (totalWidth < 0.999)
        {
            var remPct = $"{(1.0 - totalWidth) * 100:F3}%";
            sb.AppendLine($"      <td style=\"width:{remPct}\"></td>");
        }
    ...
    private string GenerateImage(BlockDto block)
    {
        ...
        string srcAttr;
        if (!string.IsNullOrEmpty(cfg.AssetId) && _assetBase64.TryGetValue(cfg.AssetId, out var dataUri))
            srcAttr = $" src=\"{dataUri}\"";
        else if (!string.IsNullOrEmpty(cfg.AssetId))
            srcAttr = $" src=\"/api/assets/{XmlAttr(cfg.AssetId)}/serve\"";
        else
            srcAttr = string.Empty;
    ...
    private static string BuildUnitCodeChoose(string unitCodeXpath)
    {
        ...
        foreach (var (code, label) in codes)
            sb.AppendLine($"  <xsl:when test=\"{unitCodeXpath}='{code}'\">{label}</xsl:when>");
        sb.AppendLine($"  <xsl:otherwise><xsl:value-of select=\"{unitCodeXpath}\"/></xsl:otherwise>");
        sb.Append("</xsl:choose>");
        return sb.ToString();
    }
